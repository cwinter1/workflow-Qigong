...
`svgFigure(inner)` wraps a `viewBox="0 0 100 140"` stick figure. Shared helpers: `bone(basePts, color, frames?, dur?)` (a `<polyline>` limb — 2+ points) and `headC(baseCx, baseCy, color, framesCx?, framesCy?, dur?)` (head circle), `groundLine()`, and stance presets `LEGS_NARROW()` / `LEGS_HORSE()`. Each movement's `illo(animated)` composes these with hand-picked coordinates for that pose — there's no generic pose parameterization, just 8 hand-tuned diagrams. Left/right limbs always originate from distinct shoulder points (`x=40` / `x=60`, not a shared `x=50` point) — sharing one origin makes two opposite-reaching limbs visually merge into a single line. "Two Hands Hold the Feet" is drawn side-on rather than front-facing, since a forward fold collapses onto one vertical line in a frontal view.

**Every `illo` takes an `animated` boolean:**
- `illo()` / `illo(false)` (used by `renderPreview`) → static single pose (first frame if the movement has more than one), exactly as before.
- `illo(true)` (used by `renderSession`, the timer screen) → loops base→frame→base→frame→base... via native SVG SMIL `<animate>` on the `points`/`cx`/`cy` attributes — no JS/CSS keyframes, works in Safari.

`frames` (the 3rd arg to `bone`/`headC`, or the `activeFrames`/`activeCxFrames` arg to the `movingBone`/`movingHead` wrappers) can be a single point-string **or an array of them**. This matters: a first pass animated every movement as a single neutral→pose loop, but real user testing showed it only ever demonstrated *half* of movements that actually alternate sides (draw the bow left vs. right, punch left vs. right, look back left vs. right, sway left vs. right) — the SMIL just cycled the one side over and over. The array form fixes that: `bone(base, color, [frameA, frameB], dur)` produces `base;frameA;base;frameB;base`, so a full loop visibly covers both sides of the real exercise. The `base` pose itself was also corrected in that same pass — several movements had been using a generic "arms hanging at sides" neutral that isn't the exercise's actual starting position (e.g. Two Hands Hold Up the Heavens starts with palms together at the belly, Drawing the Bow starts with arms crossed at the chest), which read as an incomplete/wrong movement even before the alternating-sides problem. Default loop is `2.6s`; "Bouncing on Toes and Heels" uses `1.8s` since a bounce reads better faster.
